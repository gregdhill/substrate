...
use futures::{
	future, stream, Future, FutureExt, Sink, SinkExt, Stream, StreamExt as _, TryStreamExt as _,
};
	...
	fn subscribe_runtime_version(
		...
		self.subscriptions.add(subscriber, |sink| {
			...
			sink.sink_map_err(|e| warn!("Error sending notifications: {:?}", e))
				.send_all(stream::iter(vec![Ok(version)]).chain(stream))
		...
		let initial = stream::iter(
			keys.map(|keys| {
				...
				let changes = keys
					.into_iter()
					.map(|key| {
						(key, self.client.storage(&BlockId::Hash(block), &key).ok().flatten())
					})
					.collect();
